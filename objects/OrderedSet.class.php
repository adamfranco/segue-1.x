...
class OrderedSet {
	 ...
	 */
	function isInSet (  $id ) {
		return in_array($id, $this->_items);
	}
	...
	function next () {
		...
		printpre("no more interator elements.");
		}
	}
	...
	function removeItem (  $id ) {
		...
		if (!$this->isInSet($id))
			throwError(new Error("Item specified, ".$id.", does not exist", "Set", 1));
	...
	function getPosition (  $id ) {
		...
		return array_search($id, $this->_items);
	}
