		...
		<td colspan='<? print (($_SESSION['ltype']=='admin')?"10":"9"); ?>'> 
			<table width='100%' > 
				...
				<td style='text-align: center; padding-top: 5px; border: 0px solid #FFF' valign='top'> 
					<form action="fckeditor_filebrowser.php" name='addform' method="POST" enctype="multipart/form-data"> 
...
<tr> 
	<td colspan='<? print (($_SESSION['ltype']=='admin')?"10":"9"); ?>'> 
		...
		<? if ($_SESSION['ltype'] == 'admin') print "Search all sites: <input type='checkbox' name='all' value='all sites'".(($all)?" checked='checked'":"")." />"; ?> 
		<input type='hidden' name='order' value='<? echo $order ?>' /> 
		<input type='hidden' name='editor' value='<? echo $editor ?>' /> 
		<input type='hidden' name='source' value='<? echo $source ?>' /> 
		<input type='hidden' name='comingfrom' value='<? echo $comingfrom ?>' /> 
		...
		print "<td class='td$color'>\n";
			if ($comingfrom != "viewsite") {
                ...
                // not sure where this function is called
                	...
                	//print "<input type='button' name='use' value='use' onclick=\"window.opener.SetUrl('".$url."')\" />\n";
                }   
            } else print " &nbsp; ";
...
<form action='fckeditor_filebrowser.php' name='deleteform' method='post'> 
