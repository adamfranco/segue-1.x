...
if (sitenamevalid($_REQUEST[name])) {
	if ($_REQUEST[confirm]) {
		$s->delete();
		log_entry("delete_site","$_REQUEST[name]","","","$_SESSION[auser] deleted site $_REQUEST[name]");
	} else {
...
} else log_entry("failed: delete_site","$_REQUEST[name]","","","$_SESSION[auser] deleting site $_REQUEST[name]");
