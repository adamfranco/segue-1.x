...
if (!$_REQUEST[nostatus]) {
	...
	if ($_loggedin) {	//we're already logged in
		if (!$_REQUEST[partialstatus]) {
			...
			if ($_SESSION[ltype]=='admin') {
				print " | <a href='username_lookup.php' onClick='doWindow(\"lookup\",300,300)' target='lookup' class='navlink'>user lookup</a>";
				print " | <a href='users.php' onClick='doWindow(\"users\",550,500)' target='users' class='navlink'>users</a>";
