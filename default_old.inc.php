...
if ($_loggedin) {
	...
	if ($atype=='stud') {
		...
		printc("<div class=title>Your Classes</div>");
		if (count($classes)) {
			//print class list
...
/* 	printc("</tr>"); */
	...
	if ($atype == 'prof' && count($classes)) {
		...
		$r = db_query("select * from classgroups where owner='$auser' order by addedtimestamp asc");
...
function printSiteLine($name,$ed=0,$isclass=0) {
	...
	if ($exists) $active=($a[active])?"<span class=green>(active)</span>":"<span class=red>(inactive)</span>";
	...
	printc("<span style='font-size:14px'><a href='$namelink'>".(($exists)?$a[title]:"Create this site")."</a></span>");
	printc((($active)?" $active":"")."</td></tr></table>");
